    ...
    <div className="p-6 max-w-5xl" data-testid="dashboard">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">Generate full-stack web apps from plain text</p>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-3 mb-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4" data-testid="kpi-server">
          <div className="text-xs text-zinc-500 mb-1">Backend</div>
          ...
          {health && <div className="text-xs text-zinc-600 mt-1">v{health.version}</div>}
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4" data-testid="kpi-tools">
          ...
          <div className="text-xs text-zinc-600 mt-1">{statusData?.active_builds || 0} active builds</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          ...
          <div className="text-xs text-zinc-600 mt-1">{runs.filter((r) => r.status === "running").length} active</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="text-xs text-zinc-500 mb-1">Outputs</div>
          <div className="text-lg font-bold text-zinc-100">{outputs.length}</div>
          <div className="text-xs text-zinc-600 mt-1">generated apps</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          ...
          {runs.length === 0 ? (
            <p className="text-xs text-zinc-600">No runs yet. Use Chat or start a build.</p>
          ) : (
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {runs.slice(0, 10).map((r) => (
                <div key={r.run_id} className="flex items-center justify-between text-xs">
                  <span className="text-zinc-400 truncate max-w-[200px]">{r.vibe_snippet}</span>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          ...
          {outputs.length === 0 ? (
            <p className="text-xs text-zinc-600">No outputs yet.</p>
          ) : (
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {outputs.map((o) => (
                <div key={o.name} className="text-xs">
                  <div className="text-zinc-300 font-medium">{o.project_name || o.name}</div>
                  <div className="text-zinc-500">{o.stack} &middot; {o.mtime_human}</div>
                </div>
              ))}
            </div>
          )}
