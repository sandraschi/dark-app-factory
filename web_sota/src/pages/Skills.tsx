...
export default function Skills() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      <p className="text-xs text-zinc-500 mb-4">Specialist council members available for code generation.</p>
        ...
        {specialists.map((s) => (
          <div key={s.name} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-zinc-500">{s.temperature}</span>
            </div>
            <div className="text-xs text-zinc-500 mb-1">
              {s.owned_patterns?.join(", ") || "Generalist"}
            </div>
            {s.requires?.length > 0 && (
              <div className="text-[10px] text-zinc-600">Requires: {s.requires.join(", ")}</div>
            )}
          </div>
        ))}
      </div>
